...
export default function App() {
  ...
  const [novaObra, setNovaObra] = useState<any>({ id: null, codigo_externo: '', nome: '', descricao: '', fase_atual: 'processo_inicial', data_inicio: '', data_previsao_fim: '', id_responsavel: '', valor_produto: '', valor_servico: '', observacoes: '' });
  ...
  const labelStatusDocumento = (status: string) => {
    const mapa: any = { nao_elaborado: 'Não Elaborado', em_andamento: 'Em Andamento', concluido: 'Concluído', nao_aplicavel: 'Não Aplicável' };
    return mapa[status] || status;
  };
  ...
  const buscarObras = async () => {
    if (!usuarioAtual) return;
    try {
      let query = supabase.from('obras').select('id, codigo_externo, nome, descricao, fase_atual, observacoes, data_inicio, data_previsao_fim, id_responsavel, valor_produto, valor_servico, usuarios(nome)').eq('status', 'em_andamento').order('created_at', { ascending: false });
  ...
  const buscarParcelasCliente = async (idDaObra: any) => {
    if (!idDaObra) return;
    try {
      const { data, error } = await supabase.from('parcelas_cliente').select('*').eq('id_obra', idDaObra).order('data_prevista', { ascending: true });
      if (error) throw error;
      setParcelasCliente(data || []);
    } catch (error) { console.error('Erro ao buscar parcelas do cliente:', error); }
  };

  const buscarDocumentosProjeto = async (idDaObra: any) => {
    if (!idDaObra) return;
    try {
      const { data, error } = await supabase.from('documentos_projeto').select('*').eq('id_obra', idDaObra).order('created_at', { ascending: true });
      if (error) throw error;
      setDocumentosProjeto(data || []);
    } catch (error) { console.error('Erro ao buscar documentos do projeto:', error); }
  };
    ...
    try {
      ...
      if (doc.status !== 'concluido') {
        ...
        await supabase
          ...
          .update({ status: 'concluido', indicador: 'verde', data_conclusao: doc.data_conclusao || hoje })
          .eq('id', doc.id);
      }
  ...
  const abrirArquivoDocumento = async (arquivo: any) => {
    try {
      ...
      mostrarAviso(error.message || 'Não foi possível abrir o arquivo.', 'erro');
    }
  };

  const excluirArquivoDocumento = async (arquivo: any) => {
    if (!window.confirm(`Deseja excluir o anexo ${arquivo.nome_arquivo}?`)) return;
  ...
  const buscarCronogramaObra = async (idDaObra: any) => {
    if (!idDaObra) return;
    try {
      const { data, error } = await supabase.from('cronograma_obra').select('*').eq('id_obra', idDaObra).order('ordem', { ascending: true });
      if (error) throw error;
      setCronogramaObra(data || []);
    } catch (error) { console.error('Erro ao buscar cronograma da obra:', error); }
  };
  ...
  const atualizarParcelaCliente = async (id: any, campo: string, valor: any) => {
    try {
      const payload: any = { [campo]: campo.includes('valor') ? (Number(valor) || 0) : (valor || null) };
      const { error } = await supabase.from('parcelas_cliente').update(payload).eq('id', id);
      if (error) throw error;
      setParcelasCliente(prev => prev.map(p => p.id === id ? { ...p, [campo]: payload[campo] } : p));
    } catch (error: any) { mostrarAviso(error.message, 'erro'); }
  };

  const salvarDocumentoProjeto = async () => {
    if (!obraEcoSelecionada || !novoDocumentoProjeto.item) return mostrarAviso('Informe o item do documento.', 'erro');
    setCarregando(true);
    try {
      const payload = {
        id_obra: obraEcoSelecionada.id,
      ...
      const { error } = await supabase.from('documentos_projeto').insert([payload]);
      if (error) throw error;
      setNovoDocumentoProjeto({ item: '', detalhes: '', status: 'nao_elaborado', indicador: 'vermelho', data_prevista: '', data_conclusao: '', observacao: '' });
  ...
  const deletarRegistroPMIS = async (tabela: string, id: any) => {
    ...
    try {
      ...
      if (obraEcoSelecionada) {
        if (tabela === 'parcelas_cliente') buscarParcelasCliente(obraEcoSelecionada.id);
  ...
  async function salvarObra(e: any) {
    ...
    try {
      ...
      if (novaObra.id) {
        const { error } = await supabase.from('obras').update(dadosObra).eq('id', novaObra.id);
        if (error) throw error;
        mostrarAviso('Obra atualizada!');
      } else {
        const { data: obraCriada, error } = await supabase.from('obras').insert([dadosObra]).select().single();
        if (error) throw error;

        if (obraCriada?.id) {
          const { error: erroCronograma } = await supabase.rpc('criar_cronograma_padrao_obra', { p_id_obra: obraCriada.id });
          if (erroCronograma) console.warn('Cronograma padrão não criado:', erroCronograma);

          const { error: erroDocumentos } = await supabase.rpc('criar_documentos_padrao_obra', { p_id_obra: obraCriada.id });
          if (erroDocumentos) console.warn('Documentos padrão não criados:', erroDocumentos);
        }

        mostrarAviso('Obra salva com cronograma e documentos padrão!');
      }

      setNovaObra({ id: null, codigo_externo: '', nome: '', descricao: '', fase_atual: 'processo_inicial', data_inicio: '', data_previsao_fim: '', id_responsavel: '', valor_produto: '', valor_servico: '', observacoes: '' });
      buscarObras();
            ...
            <header className="mb-6 flex flex-col md:flex-row md:items-start justify-between gap-4">
              <div>
                ...
                <div className="mt-3 flex flex-wrap gap-2 text-xs font-bold">
                  <span className="bg-[#2A6377]/10 text-[#2A6377] px-3 py-1 rounded-full">Fase: {labelFase(obraEcoSelecionada.fase_atual || 'processo_inicial')}</span>
                  <span className="bg-slate-100 text-slate-600 px-3 py-1 rounded-full">Responsável: {obraEcoSelecionada.usuarios?.nome || 'Não informado'}</span>
                  <span className="bg-slate-100 text-slate-600 px-3 py-1 rounded-full">Previsão: {formatarDataSegura(obraEcoSelecionada.data_previsao_fim)}</span>
                </div>
              </div>
            </header>
            ...
            <div className="sticky top-0 z-30 bg-slate-50/95 backdrop-blur pb-3 mb-3">
              <div className="bg-white rounded-xl border shadow-sm overflow-visible">
                ...
                {[
                  ...
                  { id: 'diario_tarefas', label: 'Diário e Tarefas', icon: ClipboardList }
                ].map(aba => {
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  <div className="bg-white p-5 rounded-xl shadow-sm border">
                    <h3 className="font-bold text-lg mb-3 flex items-center gap-2"><BookOpen size={18} className="text-[#2A6377]"/> Descrição do Projeto</h3>
                    <p className="text-slate-700 whitespace-pre-wrap min-h-[80px]">{obraEcoSelecionada.descricao || 'Nenhum descritivo cadastrado para esta obra.'}</p>
                    {obraEcoSelecionada.observacoes && <div className="mt-4 bg-slate-50 p-3 rounded-lg border text-sm text-slate-600"><strong>Observações:</strong><br/>{obraEcoSelecionada.observacoes}</div>}
                  </div>
                  <div className="bg-white p-5 rounded-xl shadow-sm border">
                    <h3 className="font-bold text-lg mb-3 flex items-center gap-2"><Activity size={18} className="text-[#2A6377]"/> Indicadores Gerais</h3>
                          ...
                          <td className="p-3 min-w-[280px]">
                            <div className="space-y-2">
                              {(arquivosDocumentos[doc.id] || []).length === 0 ? (
                                ...
                                (arquivosDocumentos[doc.id] || []).map((arquivo: any) => (
                                  <div key={arquivo.id} className="flex items-center justify-between gap-2 bg-white border rounded-lg p-2 shadow-sm">
                                    <button onClick={() => abrirArquivoDocumento(arquivo)} className="text-left flex-1 min-w-0 hover:text-[#2A6377]">
                                      <p className="font-medium text-xs truncate">{arquivo.nome_arquivo}</p>
                                      <p className="text-[10px] text-slate-400">{formatarTamanhoArquivo(arquivo.tamanho_bytes)} • {formatarDataSegura(arquivo.created_at)}</p>
                                    </button>
                                    {isAdmin && <button onClick={() => excluirArquivoDocumento(arquivo)} className="text-red-400 hover:text-red-600 shrink-0"><Trash2 size={14}/></button>}
                                  </div>
                                ))
                              )}
                              {isAdmin && (
                                <label className={`inline-flex items-center justify-center gap-2 px-3 py-2 rounded-lg text-xs font-bold cursor-pointer transition ${uploadDocumentoId === doc.id ? 'bg-slate-200 text-slate-500' : 'bg-[#2A6377]/10 text-[#2A6377] hover:bg-[#2A6377]/20'}`}>
                                  {uploadDocumentoId === doc.id ? <Loader2 className="animate-spin" size={14}/> : <Plus size={14}/>} Anexar arquivo
                                  <input type="file" className="hidden" accept=".pdf,.doc,.docx,.xls,.xlsx,.png,.jpg,.jpeg" disabled={uploadDocumentoId === doc.id} onChange={e => { const arquivo = e.target.files?.[0] || null; anexarArquivoDocumento(doc, arquivo); e.currentTarget.value = ''; }} />
                                </label>
                              )}
                            </div>
                          </td>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 flex-1 items-start">
                <div className="lg:col-span-1 space-y-6">
                  ...
                  <div className="bg-slate-50 p-4 md:p-6 rounded-xl border w-full flex flex-col items-start max-h-[600px] overflow-y-auto">
                    <h3 className="text-lg font-bold mb-6 flex items-center gap-2"><Clock size={20} className="text-slate-500" /> Histórico da Obra</h3>
                    {historicoObra.length === 0 ? <p className="text-sm text-gray-500">Sem histórico.</p> : historicoObra.map((hist, idx) => (
                      <div key={idx} className="w-full border-l-2 border-slate-200 pl-4 pb-5">
                        <h4 className="font-bold text-[#2A6377] mb-2">{hist.dataFormatada}</h4>
                        <div className="space-y-2">
                          {hist.diarios?.map((diario: any) => <div key={`d-${diario.id}`} className="bg-white p-3 rounded border text-sm"><p className="text-slate-700 whitespace-pre-wrap">{diario.texto}</p></div>)}
                          {hist.resumos?.map((res: any) => <div key={`r-${res.id}`} className="bg-white p-3 rounded border text-sm"><p className="font-bold text-slate-800 mb-1">Resumo da Reunião</p><p className="text-slate-700 whitespace-pre-wrap">{res.texto}</p></div>)}
                          {hist.ocorrencias?.map((oc: any, i: number) => <div key={`oc-${i}`} className="bg-white p-3 rounded border text-sm"><span className="font-bold text-[#2A6377]">{labelOcorrencia(oc.tipo)}:</span> {oc.descricao}</div>)}
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <div className="lg:col-span-2 flex flex-col bg-white p-5 rounded-xl shadow-sm border h-full min-h-[600px]">
                  ...
                  <div className="flex gap-4 overflow-x-auto pb-2 items-start flex-1">
                    <div className="flex-1 min-w-[260px] bg-gray-50 rounded-xl p-3 border"><div className="flex justify-between items-center mb-3"><h4 className="font-bold text-sm">A Fazer</h4><span className="bg-gray-200 text-[10px] px-2 py-0.5 rounded-full font-bold">{tarefasFiltradas.filter(t => t?.status === 'pendente').length}</span></div><div className="space-y-2">{tarefasFiltradas.filter(t => t?.status === 'pendente').map(tarefa => (<div key={tarefa?.id} onClick={() => setTarefaSelecionada(tarefa)} className="bg-white p-3 rounded shadow-sm border hover:border-[#2A6377] cursor-pointer"><p className="font-medium text-sm leading-tight mb-2">{tarefa?.titulo}</p><div className="flex justify-between items-center border-t pt-2"><span className="text-[10px] uppercase font-bold text-slate-400"><User size={10} className="inline mr-1"/>{tarefa?.usuarios?.nome}</span><span className={`text-[10px] font-bold px-1.5 py-0.5 rounded flex items-center gap-1 ${isAtrasada(tarefa?.data_vencimento, tarefa?.status) ? 'bg-red-50 text-red-600' : 'bg-gray-100 text-gray-500'}`}><Clock size={10} /> {formatarDataSegura(tarefa?.data_vencimento)}</span></div></div>))}</div></div>
                    <div className="flex-1 min-w-[260px] bg-[#2A6377]/5 rounded-xl p-3 border border-[#2A6377]/20"><div className="flex justify-between items-center mb-3"><h4 className="font-bold text-sm text-[#2A6377]">Em Andamento</h4><span className="bg-[#2A6377]/20 text-[#2A6377] text-[10px] px-2 py-0.5 rounded-full font-bold">{tarefasFiltradas.filter(t => t?.status === 'em_andamento').length}</span></div><div className="space-y-2">{tarefasFiltradas.filter(t => t?.status === 'em_andamento').map(tarefa => (<div key={tarefa?.id} onClick={() => setTarefaSelecionada(tarefa)} className="bg-white p-3 rounded shadow-sm border hover:border-[#2A6377] cursor-pointer"><p className="font-medium text-sm leading-tight mb-2">{tarefa?.titulo}</p><div className="flex justify-between items-center border-t pt-2"><span className="text-[10px] uppercase font-bold text-slate-400"><User size={10} className="inline mr-1"/>{tarefa?.usuarios?.nome}</span><span className={`text-[10px] font-bold px-1.5 py-0.5 rounded flex items-center gap-1 ${isAtrasada(tarefa?.data_vencimento, tarefa?.status) ? 'bg-red-50 text-red-600' : 'bg-gray-100 text-gray-500'}`}><Clock size={10} /> {formatarDataSegura(tarefa?.data_vencimento)}</span></div></div>))}</div></div>
                    <div className="flex-1 min-w-[260px] bg-green-50/50 rounded-xl p-3 border border-green-100"><div className="flex justify-between items-center mb-3"><h4 className="font-bold text-sm text-green-700">Concluídas</h4></div><div className="space-y-2">{tarefasFiltradas.filter(t => t?.status === 'concluida').map(tarefa => (<div key={tarefa?.id} onClick={() => setTarefaSelecionada(tarefa)} className="bg-white p-3 rounded shadow-sm border opacity-70 cursor-pointer hover:opacity-100"><p className="font-medium text-sm leading-tight mb-2 line-through text-slate-500">{tarefa?.titulo}</p></div>))}</div></div>
                  </div>
                </div>
              </div>
            )}
          ...
          <div className="animate-in fade-in dash-main-wrapper max-w-5xl">
            ...
            <form id="form-cadastro-obra" onSubmit={salvarObra} className="bg-white p-4 md:p-6 rounded-xl shadow-sm border border-gray-200 mb-6 md:mb-8 max-w-full">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 mb-6 max-w-full">
                ...
                <div><label className="block text-sm mb-1 max-w-full">Fase Atual</label><select value={novaObra.fase_atual} onChange={(e) => setNovaObra({...novaObra, fase_atual: e.target.value})} className="w-full border p-3 rounded-lg outline-none focus:border-[#2A6377] bg-white max-w-full">{fasesProjeto.map(fase => (<option key={fase.valor} value={fase.valor}>{fase.label}</option>))}</select></div>
                <div><label className="block text-sm mb-1 max-w-full">Responsável *</label><select value={novaObra.id_responsavel} onChange={(e) => setNovaObra({...novaObra, id_responsavel: e.target.value})} className="w-full border p-3 rounded-lg outline-none focus:border-[#2A6377] max-w-full bg-white"><option value="">Selecione...</option>{listaUsuarios.map(user => (<option key={user.id} value={user.id}>{user.nome}</option>))}</select></div>
                <div className="md:col-span-2"><label className="block text-sm mb-1 max-w-full">Descrição da Obra</label><textarea rows={3} value={novaObra.descricao} onChange={(e) => setNovaObra({...novaObra, descricao: e.target.value})} className="w-full border p-3 rounded-lg outline-none focus:border-[#2A6377] max-w-full" placeholder="Descreva o escopo, ambientes, objetivo do projeto etc."></textarea></div>
                ...
                <div className="md:col-span-2"><label className="block text-sm mb-1 max-w-full">Observações</label><textarea rows={2} value={novaObra.observacoes} onChange={(e) => setNovaObra({...novaObra, observacoes: e.target.value})} className="w-full border p-3 rounded-lg outline-none focus:border-[#2A6377] max-w-full" placeholder="Informações internas, premissas ou alertas."></textarea></div>
              </div>
