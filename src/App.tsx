...
export default function App() {
  ...
  const [modalHistoricoAberto, setModalHistoricoAberto] =
  ...
  const processarAuth = async (e: any) => {
    ...
    try {
      ...
      else setErroLogin(error.message);
    } finally {
      setCarregandoAuth(false);
    }
  };
  ...
  const buscarObras = async () => {
    ...
    try {
      let query = supabase
        .from('obras')
        .select(
          'id, codigo_externo, nome, data_inicio, data_previsao_fim, id_responsavel, usuarios(nome)'
        )
      ...
      try {
        ...
        if (!isAdmin) {
          ...
          const { data: obrasUsuario } = await supabase
            ...
            .select('id')
            .eq('id_responsavel', usuarioAtual.id);
          idsMinhasObras = obrasUsuario?.map((o) => o.id) || [];
        }
        ...
        let queryGrafico = supabase
          .from('tarefas')
          .select('status, obras!inner(nome)');
        if (!isAdmin) {
          if (idsMinhasObras.length > 0) {
            const condicao = `id_responsavel.eq.${
              usuarioAtual.id
            },id_obra.in.(${idsMinhasObras.join(',')})`;
        ...
        const [{ count: obrasCount }, { count: atrasadasCount }] =
          await Promise.all([queryObras, queryTarefas]);
        setResumoReal({
        ...
        if (tarefasGrafico) {
          ...
          tarefasGrafico.forEach((t: any) => {
            ...
            if (!mapaGrafico[nomeObra])
              mapaGrafico[nomeObra] = {
                nome: nomeObra,
                tarefas_concluidas: 0,
                tarefas_pendentes: 0,
              };
            if (t.status === 'concluida')
              mapaGrafico[nomeObra].tarefas_concluidas++;
            else mapaGrafico[nomeObra].tarefas_pendentes++;
          });
    ...
    async function buscarHistorico() {
      ...
      try {
        const { data, error } = await supabase
          .from('reunioes')
          .select(
            `id, data_reuniao, clima_semana, resumo_geral, ocorrencias(tipo, descricao), tarefas(titulo, data_vencimento, id_responsavel, usuarios(nome))`
          )
          .eq('id_obra', reuniaoForm.id_obra)
  ...
  const buscarTarefasKanban = async () => {
    ...
    try {
      let query = supabase
        .from('tarefas')
        .select(
          `id, id_obra, titulo, status, data_vencimento, id_responsavel, obras!inner(codigo_externo, nome, id_responsavel), usuarios(nome)`
        )
        .order('created_at', { ascending: false });
      if (!isAdmin) {
        const { data: obrasUsuario } = await supabase
  ...
  useEffect(() => {
    if (telaAtiva === 'tarefas') buscarTarefasKanban();
  }, [telaAtiva, usuarioAtual]);
  ...
  async function salvarObra(e: any) {
    ...
    try {
      ...
      if (novaObra.id) {
        ...
        const { error } = await supabase.from('obras').insert([dadosObra]);
        if (error) throw error;
        mostrarAviso('Obra salva!');
      }
      setNovaObra({
        id: null,
        codigo_externo: '',
  ...
  async function salvarReuniaoObra() {
    ...
    try {
      ...
      const responsavel = listaUsuarios.find(
        (u) => u.id === obraSelecionada.id_responsavel
      );
        ...
        .insert([
          {
            ...
            clima_semana: reuniaoForm.clima_semana,
      ...
      const registroObraAta = {
        ...
        clima: reuniaoForm.clima_semana,
      ...
      setListaTarefas([]);
      setTelaAtiva('dashboard');
      setTimeout(() => setTelaAtiva('reunioes'), 50);
    } catch (error: any) {
  ...
  const gerarAtaFinal = () => {
    ...
    obrasNaAtaAtual.forEach((obra) => {
      textoAta += `==========================================\nOBRA: ${obra.nome_obra.toUpperCase()}\n==========================================\nClima: ${
        obra.clima.charAt(0).toUpperCase() + obra.clima.slice(1)
      }\n`;
      ...
      if (obra.ocorrencias.length > 0) {
        ...
        obra.ocorrencias.forEach(
          (oc: any) =>
            (textoAta += `- (${oc.tipo.toUpperCase()}): ${oc.descricao}\n`)
        );
        textoAta += `\n`;
      }
      if (obra.tarefas.length > 0) {
        ...
        obra.tarefas.forEach(
          (t: any) =>
            (textoAta += `- ${t.titulo} (Resp: ${
              t.nome_responsavel
            } | Prazo: ${formatarDataSegura(t.data_vencimento)})\n`)
        );
        textoAta += `\n`;
      }
      textoAta += `\n`;
  ...
  const enviarPorEmailAplicativo = () => {
    const emailsAdmins = listaUsuarios
      ...
      .map((u) => u.email);
    const emailsResponsaveis = obrasNaAtaAtual
      .map((ob) => ob.email_responsavel)
      .filter(Boolean);
    const destinatarios = [
      ...new Set([...emailsAdmins, ...emailsResponsaveis]),
    ].join(',');
    const assunto = encodeURIComponent(
      `Ata de Reunião de Obras - ${formatarDataSegura(
  ...
  const copiarAta = () => {
    navigator.clipboard.writeText(ataGerada);
    mostrarAviso('Ata copiada!');
  };
            ...
            <div className="flex border-b border-slate-200 mb-6">
              <button
                ...
                className={`flex-1 pb-3 text-sm font-bold transition ${
                  modoAuth === 'login'
                    ? 'border-b-2 border-[#2A6377] text-[#2A6377]'
                    : 'text-slate-400'
                }`}
              >
                Entrar
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full flex flex-col max-h-[90vh]">
            <div className="p-6 border-b border-gray-100 flex justify-between items-center">
              <h2 className="text-xl font-bold flex items-center gap-2">
                <Clock className="text-[#2A6377]" /> Dia{' '}
                {detalhesHistorico.dataFormatada}
              </h2>
              <button
            ...
            <div className="p-6 flex-1 overflow-y-auto space-y-6">
              {detalhesHistorico.resumos.length > 0 && (
                <div>
                  <h4 className="font-bold mb-3 border-b">
                    Resumos Adicionados
                  </h4>
                  <div className="space-y-3">
                    {detalhesHistorico.resumos.map((res: any, i: number) => (
                      <div
                        key={i}
              ...
              {detalhesHistorico.ocorrencias.length > 0 && (
                <div>
                  <h4 className="font-bold mb-3 border-b">Ocorrências</h4>
                  <div className="space-y-2">
                    {detalhesHistorico.ocorrencias.map((oc: any, i: number) => (
                      <div
                        key={i}
                        className="bg-slate-50 p-3 rounded border text-sm"
                      >
                        <span className="font-bold text-[#2A6377] uppercase mr-2">
                          {oc.tipo}:
                        </span>{' '}
                        {oc.descricao}
                      </div>
              ...
              {detalhesHistorico.tarefas.length > 0 && (
                <div>
                  <h4 className="font-bold mb-3 border-b">Tarefas</h4>
                  <div className="space-y-2">
                    {detalhesHistorico.tarefas.map((tar: any, i: number) => (
                      <div
                        key={i}
                        className="bg-slate-50 p-3 rounded border text-sm flex justify-between items-center"
                      >
                        <span className="font-medium">{tar.titulo}</span>
                        <div className="flex gap-2 text-xs text-slate-500">
                          <span className="bg-white px-2 py-1 rounded">
                            <User size={12} className="inline" />{' '}
                            {tar.usuarios?.nome || 'Geral'}
                          </span>
                          <span className="bg-white px-2 py-1 rounded">
                      ...
                      <div className="flex justify-between items-center text-xs">
                        <span
                          className={`flex items-center gap-1 ${
                            isAtrasada(notif.data_vencimento, 'pendente')
                        ...
                        <button
                          ...
                          className="text-[#2A6377] font-medium"
                        >
                          Ver
                        </button>
                      </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full flex flex-col max-h-[90vh]">
            <div className="p-6 border-b border-gray-100 flex justify-between items-center">
              <h2 className="text-2xl font-bold flex items-center gap-2">
                <Mail className="text-[#2A6377]" /> Resumo da Ata
              </h2>
            ...
            <div className="p-6 border-t border-gray-100 flex justify-end gap-3">
              <button
                ...
                className="px-6 py-2 rounded-lg font-medium bg-slate-100"
              >
                Fechar
              </button>
              <button
                onClick={copiarAta}
                className="bg-slate-200 px-6 py-2 rounded-lg font-bold flex items-center gap-2"
              >
                <Copy size={18} /> Copiar
              </button>
              <button
                onClick={enviarPorEmailAplicativo}
                className="bg-[#2A6377] text-white px-6 py-2 rounded-lg font-bold flex items-center gap-2"
              >
      ...
      <aside className="w-64 bg-[#2A6377] text-white flex flex-col z-20 justify-between">
        <div>
          <div className="p-6 border-b border-white/10 flex flex-col items-center justify-center">
            ...
            <p className="text-xs text-white/80 tracking-wider font-medium uppercase mt-1">
              Gestão de Obras
            </p>
          </div>
          <nav className="p-4 space-y-2">
            ...
            <div className="pt-4 mt-2 border-t border-white/10">
              <button
                onClick={() => setPainelNotificacaoAberto(true)}
                className="w-full flex items-center justify-between p-3 rounded-lg transition hover:bg-white/10 text-white/80 hover:text-white"
              >
                <div className="flex items-center gap-3">
                  <Bell
                    size={20}
                    className={
                      minhasNotificacoes.length > 0 ? 'text-amber-300' : ''
                    }
                  />{' '}
                  Tarefas
                </div>
                {minhasNotificacoes.length > 0 && (
                  <span className="bg-red-500 text-white text-xs font-bold px-2 rounded-full animate-pulse">
        ...
        {telaAtiva === 'dashboard' && (
          <div className="animate-in fade-in">
            <h2 className="text-3xl font-bold mb-8">
              Visão Geral {isAdmin ? '(Todas)' : '(Minhas)'}
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
              <div className="bg-white p-6 rounded-xl shadow-sm flex items-center gap-4">
                <div className="p-4 bg-[#2A6377]/10 text-[#2A6377] rounded-lg">
                  <Briefcase size={24} />
                </div>
                <div>
                  ...
                  <p className="text-3xl font-bold text-[#2A6377]">
                    {resumoReal.obrasAtivas}
                  </p>
                </div>
              </div>
              <div className="bg-white p-6 rounded-xl shadow-sm flex items-center gap-4">
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm">
              <h3 className="text-lg font-bold mb-6">
                Status Real das Tarefas por Obra
              </h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              >
                ...
                <div className="flex justify-end pt-6">
                  <button
                    type="submit"
                    className="bg-[#2A6377] text-white px-6 py-2 rounded-lg font-medium"
                  >
                    ...
                    >
                      ...
                      <div>
                        <p className="font-bold text-sm">
                          {user.nome}{' '}
                          <span className="text-[10px] ml-2 px-2 py-0.5 bg-gray-200 rounded uppercase">
                            {user.perfil}
                          </span>
                        </p>
        ...
        {telaAtiva === 'obras' && (
          <div className="animate-in fade-in max-w-5xl">
              ...
              >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full border p-3 rounded-lg outline-none focus:border-[#2A6377]"
                    />
                  </div>
                  <div>
                    <label className="block text-sm mb-1">Nome *</label>
                ...
                <div className="flex justify-end pt-4 border-t">
                  <button
                    type="submit"
                    disabled={carregando}
                    className="bg-[#2A6377] text-white px-6 py-3 rounded-lg font-medium"
                  >
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
              <h3 className="text-lg font-bold mb-4 border-b pb-2">
                {isAdmin ? 'Todas' : 'Minhas'}
              </h3>
              {obrasLista.length === 0 ? (
                <p className="text-gray-500 text-sm">Nenhuma obra.</p>
              ) : (
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-slate-50 text-slate-600 text-sm border-y">
                      <th className="p-3">Código</th>
                      <th className="p-3">Nome</th>
                      <th className="p-3">Responsável</th>
                      <th className="p-3">Fim</th>
                      {isAdmin && <th className="p-3 text-right">Ação</th>}
                    </tr>
                  </thead>
                        ...
                        {isAdmin && (
                          <td className="p-3 text-right">
                            <button
                              ...
                              className="text-slate-400 hover:text-[#2A6377] p-2"
                            >
                              <Edit2 size={16} />
                            </button>
                          </td>
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border mb-6 border-l-4 border-l-[#2A6377]">
              <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                <div className="flex-1">
                  ...
                  <select
                    className="w-full max-w-lg border rounded-lg p-3 outline-none font-bold bg-gray-50"
                ...
                <div className="flex gap-3">
                  <button
                    ...
                    disabled={carregando || !reuniaoForm.id_obra}
                    className="bg-[#2A6377]/10 text-[#2A6377] px-6 py-3 rounded-lg font-bold flex items-center gap-2 disabled:opacity-50"
                  >
                    {carregando ? (
                      <Loader2 className="animate-spin" size={18} />
                    ) : (
                  ...
                  <button
                    ...
                    className="bg-[#2A6377] text-white px-6 py-3 rounded-lg font-bold flex items-center gap-2 shadow-md disabled:opacity-50"
                  >
              ...
              {obrasNaAtaAtual.length > 0 && (
                <div className="mt-6 pt-4 border-t flex flex-wrap items-center gap-2">
                  ...
                  {obrasNaAtaAtual.map((ob, idx) => (
                    <span
                      key={idx}
                      className="bg-green-100 text-green-700 text-xs font-bold px-3 py-1 rounded-full flex items-center gap-1"
                    >
                      <CheckCheck size={12} /> {ob.nome_obra}
                    </span>
                  ))}
                </div>
              )}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <div className="lg:col-span-2 space-y-6">
                <div className="bg-white p-6 rounded-xl shadow-sm border">
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-4">
                    ...
                    <div>
                      <label className="block text-sm mb-1">Clima</label>
                      <select
                        className="w-full border rounded-lg p-2 outline-none"
                        value={reuniaoForm.clima_semana}
                        onChange={(e) =>
                          setReuniaoForm({
                            ...reuniaoForm,
                            clima_semana: e.target.value,
                          })
                        }
                      >
                ...
                <div className="bg-white p-6 rounded-xl shadow-sm border">
                  ...
                  <div className="flex gap-4 mb-4">
                    <select
                      className="border rounded-lg p-2 w-1/4 outline-none"
                    ...
                    <button
                      ...
                      className="bg-slate-100 px-4 py-2 rounded-lg font-medium"
                    >
                      Add
                    </button>
                  </div>
                  {listaOcorrencias.map((oc, idx) => (
                    <div
                      key={idx}
                      className="flex justify-between items-center bg-slate-50 p-2 mt-2 rounded border text-sm"
                    >
                      <div>
                ...
                <div className="bg-white p-6 rounded-xl shadow-sm border">
                  ...
                  <div className="flex gap-4 mb-3">
                    ...
                    <input
                      type="date"
                      className="border rounded-lg p-2 w-1/3 outline-none"
                  ...
                  <div className="flex gap-4 mb-4">
                    ...
                    <button
                      ...
                      className="bg-slate-100 px-6 py-2 rounded-lg font-medium"
                    >
                      Adicionar
                    </button>
                  </div>
                  {listaTarefas.map((tar, idx) => (
                    <div
                      key={idx}
                      className="flex justify-between items-center bg-slate-50 p-3 mt-2 rounded border text-sm"
                    >
                      <div>
                        ...
                        <div className="flex gap-3 text-xs text-gray-500 mt-1">
                          <span className="flex items-center gap-1">
                            <User size={12} /> {tar.nome_responsavel}
                          </span>
                          {tar.data_vencimento && (
                            <span className="flex items-center gap-1">
                              <Clock size={12} />{' '}
                      ...
                      <button
                        ...
                        className="text-red-400 hover:text-red-600 bg-white p-2 rounded shadow-sm border"
                      >
                        <Trash2 size={16} />
                      </button>
                    </div>
              ...
              <div className="bg-slate-50 p-6 rounded-xl border">
                <h3 className="text-lg font-bold mb-6 flex items-center gap-2">
                  <Clock size={20} className="text-slate-500" /> Histórico Atual
                </h3>
                  ...
                  <div className="space-y-4">
                    {historicoObra.map((hist, idx) => (
                      <button
                        key={idx}
                        onClick={() => {
                          setDetalhesHistorico(hist);
                          setModalHistoricoAberto(true);
                        }}
                        className="w-full text-left bg-white p-4 rounded-lg border shadow-sm hover:border-[#2A6377] transition group"
                      >
                        <div className="flex justify-between items-center mb-2">
                          <span className="font-bold text-[#2A6377] flex items-center gap-2">
                            <Calendar size={16} /> {hist.dataFormatada}
                          </span>
                          <span className="text-xs bg-slate-100 px-2 py-1 rounded-full group-hover:bg-[#2A6377]/10 group-hover:text-[#2A6377]">
                            Detalhes
                          </span>
                        </div>
                        <div className="flex flex-wrap gap-3 text-xs text-slate-500 mt-2 border-t pt-2">
                          <span>{hist.resumos.length} Resumo(s)</span>
                          <span>•</span>
                          <span>{hist.ocorrencias.length} Ocorrência(s)</span>
                          <span>•</span>
                          <span>{hist.tarefas.length} Tarefa(s)</span>
                        </div>
                      </button>
                    ))}
                  </div>
          ...
          <div className="animate-in fade-in h-full flex flex-col">
            <header className="mb-6 flex justify-between items-end">
              ...
              <div className="flex items-center gap-2">
                ...
                <select
                  className="border rounded-lg p-2 outline-none font-medium bg-white shadow-sm"
            ...
            <div className="flex-1 flex gap-6 overflow-x-auto pb-4">
              ...
              <div className="flex-1 min-w-[300px] bg-gray-100/50 rounded-xl p-4 border flex flex-col">
                <div className="flex justify-between items-center mb-4">
                  <h3 className="font-bold">A Fazer</h3>
                ...
                <div className="space-y-3 flex-1">
                  {tarefasFiltradas
                    ...
                    .map((tarefa) => (
                      <div
                        key={tarefa?.id}
                        className="bg-white p-4 rounded-lg shadow-sm border hover:border-[#2A6377] transition group"
                      >
                        <div className="flex justify-between items-start mb-2">
                          <span className="text-xs font-semibold text-[#2A6377] bg-[#2A6377]/10 px-2 py-1 rounded">
                        ...
                        <div className="flex justify-between items-center border-t pt-3 mt-3">
                          ...
                          <button
                            onClick={() =>
                              atualizarStatusTarefa(tarefa?.id, 'em_andamento')
                            }
                            className="bg-[#2A6377]/10 text-[#2A6377] hover:bg-[#2A6377] hover:text-white px-2 py-1.5 rounded transition flex items-center gap-1"
                          >
                            <Play size={14} />{' '}
              ...
              <div className="flex-1 min-w-[300px] bg-[#2A6377]/5 rounded-xl p-4 border border-[#2A6377]/20 flex flex-col">
                <div className="flex justify-between items-center mb-4">
                  ...
                  <span className="bg-[#2A6377]/20 text-[#2A6377] text-xs px-2 py-1 rounded-full">
                    {
                      ...
                      <div
                        ...
                        className={`bg-white p-4 rounded-lg shadow-sm border ${
                          isAtrasada(tarefa?.data_vencimento, tarefa?.status)
                            ? 'border-red-300'
                            : 'border-gray-200 hover:border-[#2A6377]'
                        }`}
                      >
                        <div className="flex justify-between items-start mb-2">
                          <span className="text-xs font-semibold text-[#2A6377] bg-[#2A6377]/10 px-2 py-1 rounded">
                        ...
                        <div className="flex justify-between items-center border-t pt-3 mt-3">
                          ...
                          <button
                            onClick={() =>
                              atualizarStatusTarefa(tarefa?.id, 'concluida')
                            }
                            className="bg-green-100 text-green-700 hover:bg-green-600 hover:text-white px-2 py-1.5 rounded transition flex items-center gap-1 shadow-sm"
                          >
              ...
              <div className="flex-1 min-w-[300px] bg-green-50/30 rounded-xl p-4 border border-green-100 flex flex-col">
                <div className="flex justify-between items-center mb-4">
                  <h3 className="font-bold text-gray-700">Concluídas</h3>
                ...
                <div className="space-y-3 flex-1">
                  {tarefasFiltradas
                    ...
                    .map((tarefa) => (
                      <div
                        key={tarefa?.id}
                        className="bg-white p-4 rounded-lg shadow-sm border border-gray-200 opacity-70"
                      >
                        <div className="flex justify-between items-start mb-2">
                        ...
                        <p className="font-medium text-gray-500 line-through text-sm my-3">
                          {tarefa?.titulo || 'Sem Título'}
                        </p>
                        <div className="flex justify-end border-t pt-3 mt-3">
                          <div className="flex items-center gap-1 text-xs font-medium px-2 py-1 rounded bg-green-50 text-green-600">
