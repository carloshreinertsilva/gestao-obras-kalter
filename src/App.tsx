...
export default function App() {
    ...
    return (
      <div className="flex h-screen bg-slate-100 items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-xl max-w-md w-full border border-slate-200 overflow-hidden">
          ...
          <div className="bg-[#2A6377] p-8 text-center flex flex-col items-center justify-center border-b border-[#1e4857]">
  ...
  return (
    <div className="flex h-screen bg-gray-50 font-sans relative overflow-hidden">
      <div className="fixed bottom-6 right-6 z-[60] flex flex-col gap-3">{toasts.map(toast => (<div key={toast.id} className={`flex items-center gap-3 px-6 py-4 rounded-xl shadow-lg text-white font-medium ${toast.tipo === 'sucesso' ? 'bg-green-600' : 'bg-red-600'}`}>{toast.tipo === 'sucesso' ? <CheckCircle2 size={24} /> : <AlertTriangle size={24} />} {toast.mensagem}</div>))}</div>
      
      {modalHistoricoAberto && detalhesHistorico && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[70] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full flex flex-col max-h-[90vh]">
            <div className="p-6 border-b border-gray-100 flex justify-between items-center"><h2 className="text-xl font-bold flex items-center gap-2"><Clock className="text-[#2A6377]"/> Dia {detalhesHistorico.dataFormatada}</h2><button onClick={() => setModalHistoricoAberto(false)} className="text-slate-400 hover:text-red-500"><X size={24}/></button></div>
            <div className="p-6 flex-1 overflow-y-auto space-y-6">
              {detalhesHistorico.resumos.length > 0 && (<div><h4 className="font-bold mb-3 border-b">Resumos Adicionados</h4><div className="space-y-3">{detalhesHistorico.resumos.map((res: any, i: number) => (<div key={i} className="bg-slate-50 p-4 rounded-lg border"><p className="text-xs font-bold text-slate-400 uppercase mb-2">Clima: {res.clima}</p><p className="text-sm whitespace-pre-wrap">{res.texto}</p></div>))}</div></div>)}
      ...
      {painelNotificacaoAberto && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[65] flex justify-end">
      ...
      {modalAtaAberto && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[70] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full flex flex-col max-h-[90vh]">
            <div className="p-6 border-b border-gray-100 flex justify-between items-center"><h2 className="text-2xl font-bold flex items-center gap-2"><Mail className="text-[#2A6377]"/> Resumo da Ata</h2><button onClick={() => setModalAtaAberto(false)}><X size={24}/></button></div>
            <div className="p-6 flex-1 overflow-y-auto bg-slate-50"><pre className="text-sm font-mono whitespace-pre-wrap">{ataGerada}</pre></div>
            <div className="p-6 border-t border-gray-100 flex justify-end gap-3"><button onClick={() => setModalAtaAberto(false)} className="px-6 py-2 rounded-lg font-medium bg-slate-100">Fechar</button><button onClick={copiarAta} className="bg-slate-200 px-6 py-2 rounded-lg font-bold flex items-center gap-2"><Copy size={18}/> Copiar</button><button onClick={enviarPorEmailAplicativo} className="bg-[#2A6377] text-white px-6 py-2 rounded-lg font-bold flex items-center gap-2"><Send size={18}/> Abrir no E-mail</button></div>
          </div>
        </div>
      )}

      <aside className="w-64 bg-[#2A6377] text-white flex flex-col z-20 justify-between">
        <div>
          ...
          <nav className="p-4 space-y-2">
            <button onClick={() => setTelaAtiva('dashboard')} className={`w-full flex items-center gap-3 p-3 rounded-lg transition ${telaAtiva === 'dashboard' ? 'bg-white/20 text-white font-bold' : 'text-white/80 hover:bg-white/10 hover:text-white'}`}><Home size={20} /> Dashboard</button>
            {isAdmin && <button onClick={() => setTelaAtiva('equipe')} className={`w-full flex items-center gap-3 p-3 rounded-lg transition ${telaAtiva === 'equipe' ? 'bg-white/20 text-white font-bold' : 'text-white/80 hover:bg-white/10 hover:text-white'}`}><Users size={20} /> Equipe</button>}
            <button onClick={() => setTelaAtiva('obras')} className={`w-full flex items-center gap-3 p-3 rounded-lg transition ${telaAtiva === 'obras' ? 'bg-white/20 text-white font-bold' : 'text-white/80 hover:bg-white/10 hover:text-white'}`}><HardHat size={20} /> Obras</button>
            <button onClick={() => setTelaAtiva('reunioes')} className={`w-full flex items-center gap-3 p-3 rounded-lg transition ${telaAtiva === 'reunioes' ? 'bg-white/20 text-white font-bold' : 'text-white/80 hover:bg-white/10 hover:text-white'}`}><Calendar size={20} /> Reuniões</button>
            <button onClick={() => setTelaAtiva('tarefas')} className={`w-full flex items-center gap-3 p-3 rounded-lg transition ${telaAtiva === 'tarefas' ? 'bg-white/20 text-white font-bold' : 'text-white/80 hover:bg-white/10 hover:text-white'}`}><CheckSquare size={20} /> Tarefas</button>
            <div className="pt-4 mt-2 border-t border-white/10"><button onClick={() => setPainelNotificacaoAberto(true)} className="w-full flex items-center justify-between p-3 rounded-lg transition hover:bg-white/10 text-white/80 hover:text-white"><div className="flex items-center gap-3"><Bell size={20} className={minhasNotificacoes.length > 0 ? "text-amber-300" : ""} /> Tarefas</div>{minhasNotificacoes.length > 0 && <span className="bg-red-500 text-white text-xs font-bold px-2 rounded-full animate-pulse">{minhasNotificacoes.length}</span>}</button></div>
          </nav>
        </div>
      ...
      <main className="flex-1 p-8 overflow-y-auto">
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm">
              ...
              <div className="h-80 w-full">
                {dadosGrafico.length === 0 ? (
                  <div className="h-full flex items-center justify-center text-gray-400">Nenhuma tarefa.</div>
                ) : (
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={dadosGrafico}>
                      ...
                      <Bar dataKey="tarefas_concluidas" name="Concluídas" fill="#22c55e" radius={[4, 4, 0, 0]} />
                      <Bar dataKey="tarefas_pendentes" name="Pendentes" fill="#f87171" radius={[4, 4, 0, 0]} />
                    </BarChart>
                  </ResponsiveContainer>
                )}
              </div>
        ...
        {telaAtiva === 'equipe' && isAdmin && (
          <div className="animate-in fade-in max-w-4xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <form onSubmit={salvarUsuario} className="bg-white p-6 rounded-xl shadow-sm h-fit">
                <h3 className="text-lg font-bold mb-4 border-b pb-2">Novo Colaborador</h3>
                <div className="space-y-4">
                  <div><label className="block text-sm mb-1">Nome</label><input required type="text" value={novoUsuario.nome} onChange={(e) => setNovoUsuario({...novoUsuario, nome: e.target.value})} className="w-full border rounded-lg p-3 outline-none focus:border-[#2A6377]" /></div>
                  <div><label className="block text-sm mb-1">E-mail</label><input required type="email" value={novoUsuario.email} onChange={(e) => setNovoUsuario({...novoUsuario, email: e.target.value})} className="w-full border rounded-lg p-3 outline-none focus:border-[#2A6377]" /></div>
                  <div><label className="block text-sm mb-1">Perfil</label><select value={novoUsuario.perfil} onChange={(e) => setNovoUsuario({...novoUsuario, perfil: e.target.value})} className="w-full border rounded-lg p-3 outline-none focus:border-[#2A6377]"><option value="engenheiro">Engenheiro/Gestor</option><option value="admin">Administrador</option></select></div>
                </div>
                <div className="flex justify-end pt-6"><button type="submit" className="bg-[#2A6377] text-white px-6 py-2 rounded-lg font-medium"><Plus size={18} className="inline mr-2"/> Adicionar</button></div>
              </form>
              <div className="bg-white p-6 rounded-xl shadow-sm">
                <h3 className="text-lg font-bold mb-4 border-b pb-2">Registados</h3>
                <div className="space-y-3">
                  {listaUsuarios.map(user => (
                    <div key={user.id} className="flex items-center gap-3 p-3 bg-slate-50 border border-slate-100 rounded-lg">
                      <div className={`p-2 rounded-full text-white ${user.perfil === 'admin' ? 'bg-[#2A6377]' : 'bg-[#2A6377]/60'}`}><User size={16} /></div>
                      <div><p className="font-bold text-sm">{user.nome} <span className="text-[10px] ml-2 px-2 py-0.5 bg-gray-200 rounded uppercase">{user.perfil}</span></p><p className="text-xs text-slate-500">{user.email}</p></div>
                    </div>
                  ))}
                </div>
              </div>
        ...
        {telaAtiva === 'obras' && (
          <div className="animate-in fade-in max-w-5xl">
            <h2 className="text-3xl font-bold mb-8">Obras</h2>
            {isAdmin && (
              <form onSubmit={salvarObra} className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 mb-8">
                <div className="flex justify-between items-center mb-6 border-b pb-2">
                  <h3 className="text-xl font-bold">{novaObra.id ? 'Editar Obra' : 'Nova Obra'}</h3>
                  {novaObra.id && (<button type="button" onClick={cancelarEdicaoObra} className="text-gray-500 flex items-center gap-1 text-sm"><X size={16} /> Cancelar</button>)}
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                  <div><label className="block text-sm mb-1">Código *</label><input type="text" value={novaObra.codigo_externo} onChange={(e) => setNovaObra({...novaObra, codigo_externo: e.target.value})} className="w-full border p-3 rounded-lg outline-none focus:border-[#2A6377]" /></div>
                  <div><label className="block text-sm mb-1">Nome *</label><input type="text" value={novaObra.nome} onChange={(e) => setNovaObra({...novaObra, nome: e.target.value})} className="w-full border p-3 rounded-lg outline-none focus:border-[#2A6377]" /></div>
                  <div><label className="block text-sm mb-1">Início *</label><input type="date" value={novaObra.data_inicio} onChange={(e) => setNovaObra({...novaObra, data_inicio: e.target.value})} className="w-full border p-3 rounded-lg outline-none focus:border-[#2A6377]" /></div>
                  <div><label className="block text-sm mb-1">Fim *</label><input type="date" value={novaObra.data_previsao_fim} onChange={(e) => setNovaObra({...novaObra, data_previsao_fim: e.target.value})} className="w-full border p-3 rounded-lg outline-none focus:border-[#2A6377]" /></div>
                  <div className="md:col-span-2">
                    <label className="block text-sm mb-1">Responsável *</label>
                    <select value={novaObra.id_responsavel} onChange={(e) => setNovaObra({...novaObra, id_responsavel: e.target.value})} className="w-full border p-3 rounded-lg outline-none focus:border-[#2A6377]">
                      <option value="">Selecione...</option>
                      {listaUsuarios.map(user => (<option key={user.id} value={user.id}>{user.nome}</option>))}
                    </select>
                  </div>
                </div>
                <div className="flex justify-end pt-4 border-t"><button type="submit" disabled={carregando} className="bg-[#2A6377] text-white px-6 py-3 rounded-lg font-medium"><Save size={20} className="inline mr-2"/> Salvar</button></div>
              </form>
            )}
            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
              ...
              {obrasLista.length === 0 ? (<p className="text-gray-500 text-sm">Nenhuma obra.</p>) : (
                <table className="w-full text-left border-collapse">
                  <thead><tr className="bg-slate-50 text-slate-600 text-sm border-y"><th className="p-3">Código</th><th className="p-3">Nome</th><th className="p-3">Responsável</th><th className="p-3">Fim</th>{isAdmin && <th className="p-3 text-right">Ação</th>}</tr></thead>
                  <tbody className="text-sm">
                    {obrasLista.map(obra => (
                      <tr key={obra.id} className="border-b hover:bg-slate-50">
                        <td className="p-3 text-slate-700">{obra.codigo_externo}</td>
                        <td className="p-3 font-bold text-[#2A6377]">{obra.nome}</td>
                        <td className="p-3 text-slate-600">{obra.usuarios?.nome}</td>
                        <td className="p-3 text-slate-600">{formatarDataSegura(obra.data_previsao_fim)}</td>
                        {isAdmin && (<td className="p-3 text-right"><button onClick={() => editarObra(obra)} className="text-slate-400 hover:text-[#2A6377] p-2"><Edit2 size={16} /></button></td>)}
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        {telaAtiva === 'reunioes' && (
           <div className="animate-in fade-in">
             <div className="bg-white p-6 rounded-xl shadow-sm border mb-6 border-l-4 border-l-[#2A6377]">
               <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                 <div className="flex-1">
                   <label className="block text-sm font-medium mb-2">1. Selecione a Obra</label>
                   <select className="w-full max-w-lg border rounded-lg p-3 outline-none font-bold bg-gray-50" value={reuniaoForm.id_obra} onChange={(e) => setReuniaoForm({...reuniaoForm, id_obra: e.target.value})}>
                     <option value="">A carregar...</option>
                     {obrasLista.map(obra => (<option key={obra.id} value={obra.id}>{obra.codigo_externo} - {obra.nome}</option>))}
                   </select>
                 </div>
                 <div className="flex gap-3">
                   <button onClick={salvarReuniaoObra} disabled={carregando || !reuniaoForm.id_obra} className="bg-[#2A6377]/10 text-[#2A6377] px-6 py-3 rounded-lg font-bold flex items-center gap-2 disabled:opacity-50">
                     {carregando ? <Loader2 className="animate-spin" size={18} /> : <Save size={18} />} Salvar
                   </button>
                   <button onClick={gerarAtaFinal} disabled={obrasNaAtaAtual.length === 0} className="bg-[#2A6377] text-white px-6 py-3 rounded-lg font-bold flex items-center gap-2 shadow-md disabled:opacity-50">
                     <Mail size={18} /> Gerar Ata
                   </button>
                 </div>
               </div>
               {obrasNaAtaAtual.length > 0 && (
                 <div className="mt-6 pt-4 border-t flex flex-wrap items-center gap-2">
                   <span className="text-sm font-medium text-gray-500 mr-2">Salvas hoje:</span>
                   {obrasNaAtaAtual.map((ob, idx) => (<span key={idx} className="bg-green-100 text-green-700 text-xs font-bold px-3 py-1 rounded-full flex items-center gap-1"><CheckCheck size={12}/> {ob.nome_obra}</span>))}
                 </div>
               )}
             </div>
             ...
             <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
               <div className="lg:col-span-2 space-y-6">
                 ...
                 <div className="bg-white p-6 rounded-xl shadow-sm border">
                   <h3 className="text-lg font-bold mb-4 border-b pb-2">2. Resumo</h3>
                   <div className="grid grid-cols-2 gap-4 mb-4">
                     <div><label className="block text-sm mb-1">Data</label><input type="date" className="w-full border rounded-lg p-2 outline-none" value={reuniaoForm.data_reuniao} onChange={(e) => setReuniaoForm({...reuniaoForm, data_reuniao: e.target.value})}/></div>
                     <div><label className="block text-sm mb-1">Clima</label><select className="w-full border rounded-lg p-2 outline-none" value={reuniaoForm.clima_semana} onChange={(e) => setReuniaoForm({...reuniaoForm, clima_semana: e.target.value})}><option value="chuvoso">Chuvoso</option><option value="ensolarado">Ensolarado</option><option value="misto">Misto</option></select></div>
                   </div>
                   <div><label className="block text-sm mb-1">Resumo Geral</label><textarea rows={3} className="w-full border rounded-lg p-3 outline-none" value={reuniaoForm.resumo_geral} onChange={(e) => setReuniaoForm({...reuniaoForm, resumo_geral: e.target.value})}></textarea></div>
                 </div>

                 <div className="bg-white p-6 rounded-xl shadow-sm border">
                   <h3 className="text-lg font-bold mb-4 border-b pb-2">3. Ocorrências</h3>
                   <div className="flex gap-4 mb-4">
                     <select className="border rounded-lg p-2 w-1/4 outline-none" value={novaOcorrencia.tipo} onChange={e => setNovaOcorrencia({...novaOcorrencia, tipo: e.target.value})}><option value="avanco">Avanço</option><option value="atraso">Atraso</option><option value="financeiro">Financeiro</option></select>
                     <input type="text" className="border rounded-lg p-2 flex-1 outline-none" placeholder="Ex: Chegou o material..." value={novaOcorrencia.descricao} onChange={e => setNovaOcorrencia({...novaOcorrencia, descricao: e.target.value})} onKeyPress={e => e.key === 'Enter' && adicionarOcorrencia()}/>
                     <button onClick={adicionarOcorrencia} className="bg-slate-100 px-4 py-2 rounded-lg font-medium">Add</button>
                   </div>
                   {listaOcorrencias.map((oc, idx) => (
                     <div key={idx} className="flex justify-between items-center bg-slate-50 p-2 mt-2 rounded border text-sm">
                       <div><span className="font-semibold text-[#2A6377] capitalize">{oc.tipo}:</span> {oc.descricao}</div>
                       <button onClick={() => setListaOcorrencias(listaOcorrencias.filter((_, i) => i !== idx))} className="text-red-400 hover:text-red-600"><Trash2 size={16} /></button>
                     </div>
                   ))}
                 </div>

                 <div className="bg-white p-6 rounded-xl shadow-sm border">
                   <h3 className="text-lg font-bold mb-4 border-b pb-2">4. Gerar Tarefas</h3>
                   <div className="flex gap-4 mb-3">
                     <input type="text" className="border rounded-lg p-2 flex-1 outline-none" placeholder="O que precisa ser feito..." value={novaTarefa.titulo} onChange={e => setNovaTarefa({...novaTarefa, titulo: e.target.value})} />
                     <input type="date" className="border rounded-lg p-2 w-1/3 outline-none" value={novaTarefa.data_vencimento} onChange={e => setNovaTarefa({...novaTarefa, data_vencimento: e.target.value})} />
                   </div>
                   <div className="flex gap-4 mb-4">
                     <select className="border rounded-lg p-2 flex-1 outline-none" value={novaTarefa.id_responsavel} onChange={e => setNovaTarefa({...novaTarefa, id_responsavel: e.target.value})}><option value="">Atribuir a...</option>{listaUsuarios.map(u => <option key={u.id} value={u.id}>{u.nome}</option>)}</select>
                     <button onClick={adicionarTarefa} className="bg-slate-100 px-6 py-2 rounded-lg font-medium">Adicionar</button>
                   </div>
                   {listaTarefas.map((tar, idx) => (
                     <div key={idx} className="flex justify-between items-center bg-slate-50 p-3 mt-2 rounded border text-sm">
                       <div><span className="font-semibold block">{tar.titulo}</span><div className="flex gap-3 text-xs text-gray-500 mt-1"><span className="flex items-center gap-1"><User size={12}/> {tar.nome_responsavel}</span>{tar.data_vencimento && <span className="flex items-center gap-1"><Clock size={12}/> {formatarDataSegura(tar.data_vencimento)}</span>}</div></div>
                       <button onClick={() => setListaTarefas(listaTarefas.filter((_, i) => i !== idx))} className="text-red-400 hover:text-red-600 bg-white p-2 rounded shadow-sm border"><Trash2 size={16} /></button>
                     </div>
                   ))}
                 </div>
               ...
               <div className="bg-slate-50 p-6 rounded-xl border">
                 <h3 className="text-lg font-bold mb-6 flex items-center gap-2"><Clock size={20} className="text-slate-500" /> Histórico Atual</h3>
                 {historicoObra.length === 0 ? (<p className="text-sm text-gray-500">Sem histórico.</p>) : (
                   <div className="space-y-4">
                     {historicoObra.map((hist, idx) => (
                       <button key={idx} onClick={() => { setDetalhesHistorico(hist); setModalHistoricoAberto(true); }} className="w-full text-left bg-white p-4 rounded-lg border shadow-sm hover:border-[#2A6377] transition group">
                         <div className="flex justify-between items-center mb-2"><span className="font-bold text-[#2A6377] flex items-center gap-2"><Calendar size={16}/> {hist.dataFormatada}</span><span className="text-xs bg-slate-100 px-2 py-1 rounded-full group-hover:bg-[#2A6377]/10 group-hover:text-[#2A6377]">Detalhes</span></div>
                         <div className="flex flex-wrap gap-3 text-xs text-slate-500 mt-2 border-t pt-2"><span>{hist.resumos.length} Resumo(s)</span><span>•</span><span>{hist.ocorrencias.length} Ocorrência(s)</span><span>•</span><span>{hist.tarefas.length} Tarefa(s)</span></div>
                       </button>
                     ))}
                   </div>
                 )}
        ...
        {telaAtiva === 'tarefas' && (
           <div className="animate-in fade-in h-full flex flex-col">
             <header className="mb-6 flex justify-between items-end">
               <div><h2 className="text-3xl font-bold text-gray-800">Kanban</h2></div>
               <div className="flex items-center gap-2">
                 <label className="text-sm font-medium text-gray-500">Filtrar:</label>
                 <select className="border rounded-lg p-2 outline-none font-medium bg-white shadow-sm" value={filtroObraKanban} onChange={(e) => setFiltroObraKanban(e.target.value)}>
                   <option value="todas">Todas as Obras</option>
                   {obrasLista.map(o => <option key={o.id} value={o.id}>{o.codigo_externo} - {o.nome}</option>)}
                 </select>
               </div>
             </header>
             ...
             <div className="flex-1 flex gap-6 overflow-x-auto pb-4">
               {/* COLUNA: A FAZER */}
               <div className="flex-1 min-w-[300px] bg-gray-100/50 rounded-xl p-4 border flex flex-col">
               ...
               {/* COLUNA: EM ANDAMENTO */}
               <div className="flex-1 min-w-[300px] bg-[#2A6377]/5 rounded-xl p-4 border border-[#2A6377]/20 flex flex-col">
               ...
               {/* COLUNA: CONCLUÍDAS */}
               <div className="flex-1 min-w-[300px] bg-green-50/30 rounded-xl p-4 border border-green-100 flex flex-col">
